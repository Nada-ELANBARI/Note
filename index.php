...
<?php
 if(isset($_POST["btn"])  )
    ...
    $q = "select count(*) as nbr from admin where id_user = '".$_POST['user']."' and password = '".($_POST['password'])."'";
	$r = $db -> query($q);
	$c = $r -> fetch();

    $q1 = "select * from admin where id_user = '".$_POST['user']."' and password = '".($_POST['password'])."'";
	$r1 = $db -> query($q1);
	$c1 = $r1 -> fetch();
    $id_user = $c1['id_user'] ;
...
if (isset($_GET['error']))
{
  ...
  <div id="footer-box" ><p class="footer-text"><span class="sign-up" > Mot de passe oublier?</span> / <a href="inscrire.php"><span class="sign-up" > S'inscrire</span></a></p></div>
</div>
